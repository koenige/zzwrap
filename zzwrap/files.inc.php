 ...
 *
 * @param string|string[] $translate_context value of the translate_context variable
...
function wrap_tsv_translate_context($translate_context) {
	$columns = [];
	foreach (wrap_array_list($translate_context) as $pair) {
...
function wrap_lib($libraries = []) {
	...
	foreach ($libraries as $function) {
		...
		if (!$found)
			wrap_error(['The required library %s does not exist. Please install it in the `_inc/library` folder.', ['values' => '`'.$function.'`']], E_USER_ERROR);
